  ...
  <div class="sidebar" :class="{showSidebar: showSidebar}">
    <div class="sidebar-con" :class="{showbar: showSidebar}">
      <div class="head">
        <div class="avatar">
          <img
            src="https://ss2.bdstatic.com/70cFvnSh_Q1YnxGkpoWK1HF6hhy/it/u=2819344467,1935426999&fm=26&gp=0.jpg"
            alt
          />
        </div>
        <div class="name">红尘</div>
      </div>
      <div class="menu">
        <ul>
          <li>
...
export default {
  ...
  methods: {
    ...
    hidebar() {
      this.$store.dispatch('setShowSidebar',false)  //调用vuex里Actions另一种写法
    },
    showToast() {
      this.$toast('该功能未开发')
    }
